...
import django
import random
from datetime import date            for order in recent_orders:
                ...
                print(f"   • Order #{order.id}: {customer_name} - TK{order.total} - Assistant: {assistant_name}"), timedelta, time
    ...
    try:
        with transaction.atomic():
            ...
            for order in recent_orders:
                customer_name = order.customer.name if order.customer else "Walk-in"
                assistant_name = order.shop_assistant.user.get_full_name() if order.shop_assistant else "N/A"
                print(f"   • Order #{order.id}: {customer_name} - ৳{order.total} - Assistant: {assistant_name}")
